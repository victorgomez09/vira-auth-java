...
// Page page = pageRepository.findById(pageId).orElseThrow(() -> new
// NotFoundException("Page not found"));

// // Check if use have space rights
// if (spaceUserRepository.findBySpaceAndUser(page.getSpace(),
// userId).isEmpty()) {
...
// Optional<Page> parentPage =
// pageRepository.findById(requestData.getParent().getId());
// Page page = new Page();
...
// page.setTreePos(parentPage.isEmpty() ? Constants.INITIAL_TREE_POS :
// parentPage.get().getTreePos() + "10");
...
// Space space = spaceRepository.findById(spaceId).orElseThrow(() -> new
// NotFoundException("Space not found"));
